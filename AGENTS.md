...
> ⚠️ 模型归属:命理解读主链路一律走 DeepSeek v4-pro(经 ModelVerse)。早期误导性命名(如 `callClaudeAPI`)已统一改为 `callDeepSeek*`(见 `docs/必读/IMPROVEMENT-TASKS.md` T3)。**已无任何真实 Codex 调用**:`src/app/api/daily/detail-analysis` 的异常兜底原走 Codex Sonnet,现已改为重试 DeepSeek v4-pro。改动 AI 逻辑前仍以 `src/lib/ai/models.ts` 的实际端点为准。
