...
| AI | **DeepSeek v4-pro**(主,经 ModelVerse 网关)+ DeepSeek-V3.2 兜底;Codex Sonnet 仅作为「每日深度分析」的异常兜底 |
...
> ⚠️ 模型归属:命理解读主链路一律走 DeepSeek v4-pro(经 ModelVerse)。早期误导性命名(如 `callClaudeAPI`)已统一改为 `callDeepSeek*`(见 `docs/IMPROVEMENT-TASKS.md` T3)。当前代码中唯一真实调用 Codex 的是 `src/app/api/daily/detail-analysis`(每日深度分析的异常兜底,模型 `Codex-sonnet-4-20250514`)。改动 AI 逻辑前仍以 `src/lib/ai/models.ts` 的实际端点为准。
