...
- 居中显示
- 图标: loading 动画或静态太极图标
- 文字: text-[#1C1C1A16]/50, text-sm
- 底部提示: 小字说明（如"精确到分钟更准确"）

**分析完成后（有内容时）：**

结果区分为以下区块，每个区块间距 mb-8：

---

##### 区块 A — 基本信息（四柱命盘）

**标题**: "四柱命盘" — H3, 18-20px, font-semibold, text-[#1C1A16], mb-4
...
| 属性 | 说明 |
|------|------|
| 展示方式 | 定义列表（Definition List <dl>）或 2×2 网格 |
| Label 样式 | text-sm, text-[#1C1A16]/70 (secondary) |
| Value 样式 | text-sm, text-[#1C1C1A16], font-medium |
| 每柱信息 | 年柱(年干+年支+十神)、月柱、日柱(日主+日支+十神)、时柱 |
| 日主显示 | 高亮标注（如 bg-yellow-100 text-yellow-800 rounded px-1） |
| 背景 | 可选: bg-[#FAFAFA] 或透明 |
...
**标题**: "五行属性" — H3, 18-20px, font-semibold, text-[#1C1A16], mb-4
...
| Tag 样式 | pill 形 (rounded-full), px-3.5 py-1.5, text-xs~sm, font-medium |
| 配色 | 金→#F3E8FF/紫, 木→#D1FAE5/绿, 水→#DBEAFE/蓝, 火→#FEE2E2/红, 土→#FEF3C7/黄 |
| 旺/弱标注 | Tag 内小字或旁边 icon |
| 选中/高亮 | 加深底色或边框 |
...
│  AI 生成的解读正文...                               │
│                                                     │
│  （文字：text-base/leading-relaxed，color #1C1A16）  │
│                                                     │
└─────────────────────────────────────────────────────┘
```
...
| 属性 | 值 |
|------|-----|
| Tab 切换 | 3 个标签：精准命盘 / AI 解读 / 科学客观 |
| 默认激活 | AI 解读 |
| Tab 样式 | text-sm font-medium, color muted 时 bg-transparent, 选中时 bg-[#FAF9FA] + text-[#1C1A16] |
| AI 标签 | inline-flex items-center gap-1, bg-yellow-50 text-yellow-700 text-xs px-2 py-0.5 rounded |
| 正文区域 | mt-4, text-base (14-15px), leading-relaxed (1.7-1.8), color #1C1A16 |
| 最大宽度 | max-w-none（不限制，自然流动） |
| 段落间距 | 解读内部分段落之间 mb-4 |
...
| Tab | 内容 | 来源 |
|-----|------|------|
| 精准命盘 | 四柱干支 + 十神 + 纳纳 + 强弱旺计算 | 算法引擎 |
| AI 解读 | DeepSeek V3 生成的人格化解读段落 | LLM API |
| 科学客观 | 客观事实陈述（如"你的日主为 X，属 Y 五行"） | 算法提取 |

---

##### 区块 E — 全方位命理解析（六维解析）

**标题**: "全方位命理分析" — H3, 18-20px, font-semibold, text-[#1C1A16], mb-5

**桌面端 3×2 网格，移动端 2×3 或纵向堆叠**:
...
| 卡片 | 内容 | 数据来源 |
|------|------|---------|
| 性格洞察 | AI 生成 2-3 段性格描述 | LLM API |
| 事业方向 | 近 1-3 年事业趋势建议 | LLM + 八字 |
| 财运特征 | 财运类型 + 流年方向 | 八字大运 |
| 健康运势 | 易出问题部位 + 建议 | 八字 + 五行 |
| 婚姻感情 | 感情走势 + 有利时段 | 八字合化 |
| 人际关系 | 社交/合作/人缘建议 | LLM + 八字 |

**卡片规格**: 遵循 6.6 卡片规范（无边框、白底、hover 微浮 + shadow）

---

##### 区块 F — 五维运势评分

**标题**: "五维运势评分" — H3, 18-20px, font-semibold, text-[#C1A16], mb-5
...
- 圆角: rounded-full
- 数值: 百分比或 1-10 分
- 维度名: text-xs text-muted 在左侧, 分数在右侧
