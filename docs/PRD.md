...
| **即将上线态** | opacity-50 + 禁止点击 + 右上角小 Tag | `opacity-50 pointer-events-none` |
| 图标 | text-2xl, 居中 | |
| 标题 | text-sm font-medium, text-center, mt-2 | |
| 描述 | text-xs text-muted, text-center, mt-1 | line-clamp-1 |

> ⚠️ **重要设计修正（2026-04-08 Frank 反馈）：**
> - **选中态禁止使用纯黑底 `bg-[#1C1A16] text-white`**，与全站暖米白风格冲突太强
> - 改用 **边框高亮 + 极淡暖底填充**（同全站 Design Tokens v6 选中态规范）
> - 即将上线功能不要只显示"即将上线"文字，需配合视觉弱化（opacity 50% + 禁止交互）

**即将上线态详细处理：**

| 属性 | 处理方式 |
|------|---------|
| 整体透明度 | `opacity-50`（视觉上明显变淡但不消失） |
| 点击事件 | `pointer-events-none`（不可选中） |
| 标识标签 | 卡片右上角小 Tag："即将上线"，bg-gray-100 text-gray-400 text-[10px] px-2 py-0.5 rounded-full |
| 图标/文字 | 保持原色不变（不置灰单独元素，靠整体 opacity 统一降级） |
| Hover | 无 hover 效果（已禁用交互） |

> **产品理由：** 用户看到"即将上线"知道功能在路上了，不会以为 bug；opacity 弱化让用户注意力自然落在可用选项上。等对应功能开发完成后去掉 opacity 和 tag 即可。

**交互逻辑：**
- 切换起卦方式 → 下方的「④ 起卦参数区」动态切换内容
- 默认选中「手动起卦」（六爻最传统的方式，也是当前唯一可用的）
- 点击"即将上线"卡片无反应（已禁用）
- P0 阶段只有手动起卦可用，其余三种展示为即将上线
...
| 默认值 | 当前时间 |
| 格式 | YYYY-MM-DD HH:mm |
| 说明文字 | "基于时间数字（年月日时）按易数规律自动生成卦象" |
| 起卦逻辑 | 年+月+日+时的数字按先天伏羲数取模得卦 |
