...
#### 7.1.4 分析系统功能区（核心功能展示区，参考 FateMaster）

**位置**: 核心理念区下方

**这是首页最核心的区域，按功能分类分组展示所有功能入口。**

**区块标题**: `分析系统` — H2 级别, 28-32px, font-semibold, text-center, margin-bottom 12px

**区块副标题**: `探索我们全面的智能分析服务` — Body Small, 14px, text-brand-gray, text-center, margin-bottom 48px

**布局**: 按功能类别分组，每组内 3 列卡片网格

##### 分组 1：八字分析（3 个功能）

**组标题**: `八字分析` — H3, 20px, font-semibold, text-left, margin-bottom 16px, mt-32 (首次 mt-0)
...
| 功能 | 描述文案 | 路径 |
|------|---------|------|
| 命盘解析 | AI 智能八字分析系统，揭示个人命盘特质与发展规律 | /bazi |
...
| 功能 | 描述文案 | 路径 |
|------|---------|------|
| 每日决策 | 基于梅花易数的每日决策指导，助你做出重要选择 | /meihua |
| 六爻占卜 | 传统六爻预测，结合 AI 深度解析卦象变化 | /liuyao（如有）|
| 塔罗解读 | 塔罗牌参考，配合 AI 观察，从不同角度思考当前处境 | /tarot |

> 注：六爻占卜如果尚未开发，可暂时隐藏或标注"即将上线"
...
| 功能 | 描述文案 | 路径 |
|------|---------|------|
| 紫微排盘 | 根据出生时间排出紫微命盘，分析十二宫位与主星格局 | /ziwei |
| 黄历查询 | 传统中国黄历，查询各类活动的吉日 | /huangli |
| AI 老黄历 | AI 驱动的智能黄历，结合现代生活场景给出建议 | /huangai（或合并到 /huangli）|

> 注：如果某些功能尚未上线，对应卡片可显示为灰色 + "即将上线"标签
...
- 边框: 无或 1px solid #F3F4F6
- Hover: `transform: translateY(-4px)` + shadow 加深 (`shadow-card-hover`)
- 过渡: `transition-all duration-300 ease`
- 图标: 40-48x40-48px, color #374151
- 标题: H3, 18-20px, font-medium, #1A1A1A, margin-top 20px (mt-5), margin-bottom 8px (mt-2)
- 描述: 14px, #6B7280, line-height 1.6, margin-bottom 16px (mb-4)
- 链接: Text 按钮, 14px, #1A1A1A, 带 arrow icon "→", hover underline
- 内容垂直排列: flex flex-col, items flex-start（不是居中，左对齐更像 FateMaster）

**Grid 规格**: 每组内 `grid grid-cols-1 md:grid-cols-3 gap-6`, max-width 1000px mx-auto

**组间距**: 每个分组之间 margin-top 48-64px (mt-12~16)
