...
| P1-3 | 无操作引导 | 中 | 用户看完不知道能做什么，缺少 CTA |
...
```
┌─────────────────────────────────────────────────────┐
│  A 区 — 命盘核心区（首屏必看，一屏内展示完）         │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐            │
│  │ 四柱命盘   │ │ 五行属性   │ │ 日主摘要   │            │
│  │ (4卡片)    │ │ (圆环+Tag)│ │ (一句话)   │            │
│  └──────────┘ └──────────┘ └──────────┘            │
├─────────────────────────────────────────────────────┤
│  B 区 — AI 解读区（Tab 切换，按需查看）              │
│  ┌─ Tab栏 ──────────────────────────────────────┐  │
│  │ 性格 │ 事业财运 │ 婚姻健康 │ 大运流年        │  │
│  ├───────────────────────────────────────────────┤  │
│  │                                               │  │
...
**设计原则：**
1. 首屏给结论，不给过程 — 用户最关心"我是什么命"
2. 信息分层折叠 — 详情按需展开，不强制全量展示
3. 每个区块有明确行动点 — 看完知道下一步做什么
4. 历史记录入口显性化 — 不藏在菜单里
...
这是用户看到结果的**第一眼**，必须在**一屏内完整展示**。

##### A.1 四柱命盘卡片组

沿用现有 7.2.5 区块 A 设计（4 卡片横排），微调：
...
| 属性 | 值 | Tailwind |
|------|-----|----------|
| 容器 | 全宽卡片 | `w-full bg-white rounded-2xl p-6` |
| 背景 | 渐变暖底 | `bg-gradient-to-r from-amber-50/50 to-orange-50/50` |
| 边框 | 左侧 4px 品牌色竖线 | `border-l-4 border-[#1C1A16]` |
| 标题字号 | text-lg (18px), font-semibold | `text-lg font-semibold text-[#1C1A16]` |
| 正文 | text-sm, leading-relaxed | `text-sm text-[#1C1A16]/75 leading-relaxed mt-2` |
| 喜忌神 | 行内 Tag | `inline-flex text-xs rounded-full px-2 py-0.5` |
| 喜用神 Tag 色 | 绿色系 | `bg-green-50 text-green-700` |
| 忌神 Tag 色 | 红色系 | `bg-red-50 text-red-700` |
...
| 属性 | 值 |
|------|-----|
| 时间轴 | 横向可滑动卡片列表 `overflow-x-auto flex gap-3 snap-x` |
...
| 按钮 | 类型 | 行为 | 优先级 |
|------|------|------|--------|
| 保存命盘 | Ghost | 登录后保存到历史记录 | P0 |
| 分享结果 | Ghost | 生成分享图片/链接 | P1 |
| 重新测算 | Ghost | 清空结果回到输入态 | P0 |
| **查看历史** | **Ghost + 箭头** | **跳转 /history 页面** | **P0（新增）** |

按钮规格同 7.2.5 区块 G（Ghost 次按钮），"查看历史"按钮额外加右箭头图标 `→`。
...
> **状态**: 新功能，v3 改版一并实现
> **路由**: `/history`
> **入口**: (1) 八字结果页底部操作区 (2) 导航栏/Footer (3) 首页
...
| 优先级 | 功能 | 说明 |
|--------|------|------|
| **P0** | 结果页 A/B/C 三区重构 | 命盘核心 + AI Tab解读 + 操作引导 |
| **P0** | 日主摘要卡（A.3） | 一句话概括，差异化亮点 |
| **P0** | 历史记录基础功能 | localStorage 存储 + /history 页面 + 保存/查看/删除 |
| **P0** | 大运流年时间轴 | 横向滑动卡片 |
| **P1** | 五行圆环图（A.2） | SVG 动画，提升视觉 |
| **P1** | 记录命名 | 自定义命盘名称 |
