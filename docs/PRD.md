...
## 附录 A：每日运势页配色优化方案（v1 — 2026-05-29）

> **状态**: 待代码虾执行
> **作者**: 产品虾 🦐
> **适用页面**: `/daily` 每日运势页
> **基准**: Design Tokens v6（第六章），本附录仅做语义重映射，不破坏全站 token 体系
...
| # | 问题 | 位置 | 优先级 |
|---|------|------|------|
| 1 | 深藏蓝「今日卦象」模块色温与全站暖米白断层 | 卦象 CTA 区块 | P0 |
| 2 | 「分享今日运势」按钮深蓝底与全站 `#1C1A16` 黑底体系不一致 | 顶部/底部 CTA | P0 |
| 3 | 五行卡片活跃态冷蓝/冷绿与暖系底色违和 | 五行选择器 | P1 |
| 4 | 五维运势进度条全橙，五维无语义区分 | 五维运势条 | P1 |
| 5 | 幸运指南四列色彩权重相同，时间敏感信息「吉时」无突出 | 幸运指南区 | P2 |
...
```
现在: 深藏蓝 ~#2D2B6B（冷色，色温断层）
改为: 深暖棕 #1C1208（暖黑系延伸，书卷感，与全站统一）

模块样式:
  background: #1C1208
  border: 1px solid rgba(200, 162, 42, 0.2)   /* 金色描边淡入 */
  border-radius: 沿用原值

内部按钮（开始占卜）:
  background: #C8A22A   /* 暗金，保留神秘感 */
  color: #1C1A16        /* 暖黑文字 */
  hover: #C8A22A/90
```
...
改为: 统一 v6 主按钮规范
  ...
  color: #FFFFFF
  border-radius: 沿用原值
  hover: rgba(28,26,22,0.90)
  icon: 分享箭头图标 16px，white，按钮文字前
```
...
活跃态（五行各异）:
  木: bg #EEF4E8 | text #4A7A35 | border rgba(74,122,53,0.20)
  火: bg #FDF0E8 | text #C8622A | border rgba(200,98,42,0.20)
  土: bg #F5F0E0 | text #8B6914 | border rgba(139,105,20,0.20)
  金: bg #F5F0E8 | text #8B7340 | border rgba(139,115,64,0.20)
  水: bg #E8EEF5 | text #2A5C8B | border rgba(42,92,139,0.20)
```

#### A.2.4 五维运势进度条

```
轨道背景（所有）: rgba(28, 26, 22, 0.06)
圆角: 4px
...
```
仅「吉时」一列:
  background: rgba(200, 162, 42, 0.08)  /* 极淡金底 */
  border-radius: 6px
  padding: 沿用原值
  label 颜色: #8B6914  /* 暗金 */

其余三列（幸运色/数字/方位）: 保持原中性样式不变
```
...
- 页面底色 `#FAF9F6` **不变**
- 主文字 `#1C1A16` **不变**
- 边框系统 `rgba(28,26,22,0.06~0.14)` **不变**
- 字体系统（Cormorant Garamond / Noto Serif SC / Inter）**不变**
- 卡片圆角、间距、布局结构 **不变**
- Design Tokens v6 其他页面 **不变**（本附录仅影响 `/daily`）

---

### A.4 验收标准

- [ ] 今日卦象模块背景为 `#1C1208`，无冷蓝色残留
- [ ] 分享按钮与首页主 CTA 按钮视觉一致（`#1C1A16` 底）
- [ ] 五行卡片活跃态五色正确对应，非活跃态统一极淡灰
- [ ] 五维进度条五色区分，无全橙
- [ ] 幸运指南「吉时」有淡金底高亮，其余三列无变化
- [ ] 页面整体色温统一为暖系，无冷蓝色断层感

---

*本附录由产品虾 🦐 出具于 2026-05-29，Frank 口头确认执行。*

---

## 附录 A.v2：今日卦象模块重做（2026-05-29）

> **背景**：A.v1 上线后 Frank 反馈「太丑」——`#1C1208` 深暖棕背景在米白底上太重，金黄按钮变成芙末黄，戏曲海报感。现改为书卷羚皮纸风格。
>
> **范围**：仅覆盖附录 A.2.1 「今日卦象模块」一项，其余 A.2.2~A.2.5 保持 v1 不变。
>
> **状态**：Frank 拍板选 A 方案，待代码虾执行。

---

### A.v2.1 今日卦象模块（重做）

**设计思路**：让卦象模块像一张「羚皮纸」嵌在页面里，不抢戏，有书卷气，按钮回归全站 v6 主按钮规范。

```
背景: #F5EFE2          /* 极淡米黄，比页面底色 #FAF9F6 略深 */
边框: 1px solid rgba(28, 26, 22, 0.08)   /* 极淡暖黑描边 */
圆角: 沿用原值（16px 或页面卡片统一规格）
阴影: 无（保持平面感）

内部文字:
  标题（今日卦象）: #1C1A16  暖黑
  描述文字: rgba(28, 26, 22, 0.55)   /* 次要说明 */

按钮（开始占卜 / 查看卦象）:
  background: #1C1A16            /* 暖黑，统一 v6 主按钮 */
  color: #FFFFFF
  hover: rgba(28, 26, 22, 0.90)
  圆角: 沿用 v6 按钮规范

可选装饰（如果原 UI 有）:
  卦象图标（魁星太极 / 六爯）: rgba(28, 26, 22, 0.35)  极淡描述色
  金色仅作为点缀，不作大面积色块
```
...
原 A.4 验收清单中的「今日卦象模块背景为 `#1C1208`」改为：
- [ ] 今日卦象模块背景为 `#F5EFE2` 极淡米黄，边框为 `rgba(28,26,22,0.08)`
- [ ] 卦象模块内按钮为 `#1C1A16` 暖黑底白字（与顶部分享按钮视觉一致）
- [ ] 模块与下方「今日之歌」卡片过渡自然，无断崖色块不连续感
- [ ] 以人眼验收为准：整体页面像「书卷气质」，不要「戏曲海报感」

---

*本 v2 补充由产品虾 🦐 出具于 2026-05-29，Frank 拍板方案 A。*

---

## 附录 A.v3：/daily 按钮体系分级修正（2026-05-29）

> **背景**：v2 上线后 Frank 反馈「黑色按钮太丑」。v6 主按钮 `#1C1A16` 被机械执行到了「今日卦象」「分享」「八字引导」等引导型 CTA上，造成三个大黑块在书卷页面上对撞，没书卷气。
>
> **根本问题**：v6 主按钮是为「决断时刻」（登录/注册/付费/提交表单）准备的，不适用于「辅助引导」。按钮需要分级。
>
> **状态**：Frank 拍板「宝纸描边」方向，待代码虾执行。

---

### A.v3.1 按钮分级体系（全站适用原则）

**一级按钮（决断型，保持 v6 原状）**

使用场景：登录 / 注册 / 付费 / 提交表单 / 退出确认
...
使用场景：页面内功能引导 / 模块中间 CTA / 分享 / 查看详情 / 跨页跳转

```
background: transparent  /* 或 #FAF9F6，透优先 */
color: #1C1A16
border: 1.5px solid rgba(28, 26, 22, 0.20)
hover background: rgba(28, 26, 22, 0.04)  /* 极淡暖底填充 */
hover border: rgba(28, 26, 22, 0.35)
圆角: 沿用原值
内边距: 沿用原值
```

视觉比喻：二级按钮像「宝纸上描了一道淡墨线」，不抢戏但点击区清晰。

---

### A.v3.2 /daily 页面中需改为二级按钮的位置

| # | 位置 | 现状 | 改为 |
|---|------|------|------|
| 1 | 顶部「📤 分享今日运势」 | `#1C1A16` 黑底白字 | 二级按钮 | 
| 2 | 今日卦象模块「开始占卜 →」 | `#1C1A16` 黑底白字 | 二级按钮 |
| 3 | 底部「📤 分享今日运势」 | `#1C1A16` 黑底白字 | 二级按钮 |
| 4 | 「八字全面分析 →」 | `#1C1A16` 黑底白字 | 二级按钮 |

**保留一级按钮**：/daily 页面本身不动页面外的登录按钮、导航栏注册按钮。

---

### A.v3.3 可选：卦象图标优化

当前「今日卦象」模块中的 `≡` 符号像象菜单图标，不贴主题。

**建议（不强制）**：
- 换为八卦符号 `☰`（乾） / `☷`（坤）
- 或装饰星 `✦`
- 或使用太极图 SVG 图标（极淡描述色 rgba(28,26,22,0.35)）

这一项代码虾可自行判断，如果我查阅圣誓符集后不合适可跳过。

---

### A.v3.4 验收标准

- [ ] /daily 页面 4 个引导型 CTA 全部改为二级按钮（透明底 + 淡描边）
- [ ] hover 效果有极淡暖底填充，描边加深
- [ ] 页面整体不再出现「大面积黑色块」
- [ ] /daily 以外的页面主按钮（登录/注册/付费）保持 v6 暖黑不动
- [ ] 以人眼验收为准：页面书卷气质，不抢戏但点击区清晰

---

*本 v3 补充由产品虾 🦐 出具于 2026-05-29，Frank 拍板「宝纸描边」方向。*

---

## 附录 A.v4：/daily 对标参考图重构（2026-05-29 下午）

> **背景**：Frank 发来一张高度成熟的「今日运势」参考图，资讯密度/色彩纪律/信息层级都优于现状。作出逐模块比对后，Frank 拍板按参考图方向优化。
>
> **范围**：/daily 页面全部核心模块，分两批执行。
>
> **状态**：Frank 拍板，待代码虾执行第一批。

---

### A.v4.0 参考图核心设计要点

1. **「判词」为顶部魂**：4 字关键词（例「宜守不宜攻」） + 一句话解读
2. **五维运势全橙 + 0-100 数值 + 综合分**
3. **五行每个都标等级**：衰/中/旺/平 四级
4. **今日卦象是米白底**：橙色描边 + 橙文字小按钮
5. **八字深度分析卡片化**：小卡片 + 橙色「查看」小按钮
6. **全场仅赭橙一色**：除宜忌绿红极小标签外
...
卡片结构：
- 顶部一行：左 「2026-05-29 · 甲子日 · 农历四月初七」 / 右 「日柱 甲子」淡描述色小标签
- 中部主区：左 大圆【低/平/高】赭橙实心直径 80px / 右上 判词 4 字加粗 #1C1A16 18-20px / 右下 一句话解读 rgba(28,26,22,0.65) 14px
- 下部五维区（同卡片内）：事业/财运/情感/健康/人缘/综合，全赭橙进度条 + 右侧 0-100 数值，综合加粗
- 边框 1px solid rgba(28,26,22,0.08)，底色 #FFFFFF 或 #FAF9F6

**判词字段需求**：后端返回 `headline`（4字） + `summary`（<=40字）。如未返回则前端 hardcode mock fallback。代码虾必读后端代码确认引擎能力。

#### 改动 2：五维运势全橙 + 数值

- 进度条颜色统一 #C8622A 赭橙，轨道 rgba(28,26,22,0.06)，圆角 4px
- 右侧添加数值 0-100 整数，14px，#1C1A16
- 综合分独立一行，加粗变调
- 后端需返回每维度 0-100 分值 + 综合分，不能输出走 mock

#### 改动 3：五行模块等级补全

每个五行方块下方加等级标签：衰/中/旺/平/相五级。

- 活跃态（旺）保留 A.v1 五色暖系不变
- 非活跃态：bg rgba(28,26,22,0.03) / text rgba(28,26,22,0.45) / border rgba(28,26,22,0.08)
- 等级标签 rgba(28,26,22,0.45)，12px

后端需返回 5 个五行等级，不能返回走 mock。

#### 改动 4：今日卦象改米白底

- 背景 #FFFFFF 或 #FAF9F6（与周围卡片一致）
- 边框 1.5px solid rgba(200,98,42,0.30)  赭橙描边
- 头部：左【第29卦】淡描述色小标签 + 右「易」字小章
- 中部大字：「坎为水」24px Cormorant Garamond + Noto Serif SC，#1C1A16
- 卦辞：#C8622A 赭橙 斜体
- 解读：3 行，rgba(28,26,22,0.65)，14px
- **按钮改为文字按钮**：「展开详解 →」，color #C8622A，font-weight 500，hover 下划线
- 明令禁止：#1C1208 深棕 / #1C1A16 暖黑按钮 / #F5EFE2 米黄底

#### 改动 5：八字深度分析卡片化

小卡片三区布局：
- 左：八卦图标 SVG 32px，rgba(28,26,22,0.35)
- 中：主标题「八字深度分析」#1C1A16 16px 加粗 / 副标题「解锁全年运势走向」rgba(28,26,22,0.55) 13px
- 右：赭橙小按钮 「查看」 bg #C8622A / color #FFFFFF / padding 8px 16px / radius 6px / 13px
- 卡片：bg #FFFFFF，border 1px solid rgba(28,26,22,0.08)，内边距 16-20px

---

### A.v4.2 第二批改动（第一批验收后启动）

- 宜忌检查是否已左右双栏，否则改为双栏 + 绿红小标签
- 今日幸运去装饰化：取消吉时淡金底高亮，四列等宽中性显示
- 全场分享按钮：底部「分享今日运势」从宝纸描边改为赭橙文字按钮

---

### A.v4.3 验收标准（第一批）

- [ ] 头部核心速览卡包含：日期·判词·一句话解读·大圆运势·五维体·综合分
- [ ] 五维进度条全部赭橙，右侧有 0-100 数值，综合加粗
- [ ] 5 个五行都显示等级标签（衰/中/旺/平/相）
- [ ] 今日卦象是米白底 + 赭橙描边 + 橙色文字「展开详解 →」按钮
- [ ] 八字深度分析从大按钮 → 小卡片 + 赭橙「查看」小按钮
- [ ] 如后端不输出判词/数值/五行等级，代码虾在 PR 描述中明确标出 mock 字段

---

*本 v4 补充由产品虾 🦐 出具于 2026-05-29，Frank 拍板「对标参考图」方向。*
