...
```
┌──────────────────────────────────────────────┐
│  📋 订阅管理                        查看详情 → │  ← 跳转 /pricing?tab=manage
│                                              │
│  当前计划：专业版（季卡）                     │
│  ¥69/季                                      │
│  状态：✅ 有效中                               │
...
**「升级套餐」按钮行为**：
- 点击后跳转 `/pricing`，URL 带 `?current=quarterly` 参数
- 定价页识别该参数后：
  - 高亮当前套餐（显示「当前计划」标签）
  - 推荐更高档的套餐（如从季卡推荐年卡）
  - CTA 文案变为「升级」而非「立即开通」
...
**「管理订阅」按钮行为**：
- 点击展开订阅管理面板（或在 `/pricing?tab=manage` 展示管理区），包含：

| 功能 | 说明 | 优先级 |
|------|------|--------|
| 查看账单历史 | 过往支付记录、发票下载 | P0 |
...
| 生效时机 | **立即生效** |
| 计费方式 | 按剩余天数比例补差价 |
| 示例 | 季卡用了 30 天（还剩 60 天），升级到年卡 → 补 (年卡价 - 季卡价) × (60/90) |
| 原周期 | 作废，新周期从升级日起算完整周期 |
| UI 交互 | 点击更高档卡片的「升级 →」→ 弹出确认弹窗 → 显示补差金额 → 确认后调起支付 |
...
│  升级到 尊享版（年卡）                   │
│                                        │
│  需补差价：¥ 130.00                    │  ← 明确显示补差金额
│                                        │
│  新周期：2026-04-15 ~ 2027-04-14       │  ← 新的完整周期
│                                        │
│  ☐ 我理解升级后将立即生效并扣费        │  ← 强制勾选
│                                        │
│  [ 取消 ]              [ 确认并支付 ¥130 ]│
...
| 项目 | 规则 |
|------|------|
| 生效时机 | **当前周期结束后生效**（不立即生效） |
| 计费方式 | 到期日按新套餐续费，不退差价 |
| UI 交互 | 点击更低档卡片的「切换 ↓」→ 弹出确认弹窗 → 说明生效时间 → 确认后预约降级 |
...
**降级期间用户可见性**：
- 降级预约后，订阅卡片显示：「⏳ 将于 YYYY-MM-DD 切换为基础版」
- 提供「撤销降级」按钮（在生效前随时可取消降级操作）
...
Request: { "new_plan": "yearly" }
Response:
{
  "ok": true,
  "prorated_amount": 130.00,    // 补差价（升级时）
  "effective_date": "2026-04-15", // 升级=即时 / 降级=到期日
  "requires_payment": true,
  "payment_intent_client_secret": "pi_xxx"  // Stripe PaymentIntent
}
...
| 新增 | PlanSwitcher | components/profile/PlanSwitcher.tsx | 套餐切换卡片组（升级/降级） |
...
| 新增 | UpgradeConfirmModal | components/pricing/UpgradeConfirmModal.tsx | 升级确认弹窗（补差价） |
| 新增 | DowngradeConfirmModal | components/pricing/DowngradeConfirmModal.tsx | 降级确认弹窗 |
| 新增 API | app/api/subscription/current/route.ts | 查询当前订阅 |
| 新增 API | app/api/subscription/change-plan/route.ts | 变更套餐 |
...
| 更低档套餐 | CTA 文案为「切换」（确认后到期后生效或立即按比例退差价）|
| 更高档套餐 | CTA 文案为「升级 ✓」（升级即时生效，补差价）|
