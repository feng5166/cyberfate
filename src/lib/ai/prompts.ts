...
- 动爻主次分离（重要）：
  「动爻」：100-150 字，必须说清三点：① 该爻对用神是生助还是制约 ② 变化后的新状态是什么 ③ 对用户问题的实际影响是利是阻
  「静爻」：30-50 字，点到即止，说明此爻对用神有辅助或无特殊影响即可，不凑字数
- overallNarrative 控制在 700-900 字，严格分四个层次展开：
  ① 用通俗语言总结卦象对用户问题的整体判断，明确「利/不利/需等待」（80字）
  ② 针对用户问题的具体分析——当前处境、主要制约、有利条件（200字）
  ③ 动爻带来的近期变化方向——最活跃的影响因素是什么，会往哪个方向发展（150字）
  ④ 应期与节奏建议——具体说明「大概什么时间窗口最有利」（用日/周/月表述），「出现什么信号代表时机成熟」（250字，此层是六爻最有实用价值的信息，必须写足）；列举多个信号时每条独立一行，用 ① ② ③ 标注，每条之间用 \n 分隔，禁止挤在同一行用分号隔开
